...
test('drink package upgrade flow — picker then real charge', async ({ page }) => {
  ...
  await expect(page.getByText(/\$300.*\$350.*credit/i).first()).toBeVisible({ timeout: 8000 });
  await expect(page.getByText(/\$500.*\$600.*credit/i).first()).toBeVisible({ timeout: 2000 });
